...
export function TicketCard({
  ...
  const handleDeleteTicket = async () => {
    ...
    const { error } = await supabase.from('tickets').delete().eq('id', ticket_id);
  ...
  return (
    <>
      <ContextMenu>
            ...
            <CardFooter className="flex flex-col items-start space-y-1 text-xs text-muted-foreground">
              ...
              {closed_at && (
                <div className="flex items-center">
                  ...
                  <time dateTime={closed_at}>{dateTimeGenerator(closed_at)}</time>
                </div>
              )}
            </CardFooter>
          </Card>
        ...
        <ContextMenuContent>
          {userRole === 'end_user' ? (
            <ContextMenuItem onClick={() => setEditDialogOpen(true)}>Edit Ticket</ContextMenuItem>
          ) : (
            <>
              ...
              {userRole === 'head_it' && (
                <ContextMenuItem onClick={() => setDeleteAlertOpen(true)} className="text-red-600">
                  Delete Ticket
                </ContextMenuItem>
              )}
            </>
      ...
      <AlertDialog open={alertOpen} onOpenChange={setAlertOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to change the status of this ticket from{' '}
              {is_closed ? 'Closed' : 'Pending'} to {is_closed ? 'Pending' : 'Closed'}?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={changeTicketStatus}>Confirm</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
